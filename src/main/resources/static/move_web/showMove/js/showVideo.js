$(document).ready(function () {
    ...
    function playVid() {
        if (video.get(0).paused) {
            video.get(0).play();
            playIcon.css({
                'background': 'url(/move_web/showMove/images/pause.png)',
            ...
            video.get(0).pause();
            playIcon.css({
                'background': 'url(/move_web/showMove/images/play.png)',
                'background-size': '100% 100%'
            });
            overlayScreen.show();
    ...
    function updateplayer() {
        ...
        if (video[0].ended) {
            ...
            overlayScreen.show();
            overlayButton.css({
                'background': 'url(/move_web/showMove/images/replay.png)',
                'background-size': '100% 100%'
            });
        } else if (video[0].paused) {
            overlayButton.css({
                'background': 'url(/move_web/showMove/images/play-button.png)',
                'background-size': '100% 100%'
            });
        }
    }
    ...
    function toggleMute() {
        if (!video[0].muted) {
            video[0].muted = true;
            volume.css({
                'background': 'url(/move_web/showMove/images/mute.png)',
                'background-size': '100% 100%'
            });
            intensity.hide();
        } else {
            video[0].muted = false;
            volume.css({
                'background': 'url(/move_web/showMove/images/volume.png)',
                'background-size': '100% 100%'
            });
            intensity.show();
        }
